...
void OnDataRecv(const uint8_t *mac, const uint8_t *incoming_data, int len)
{
  ...
  {
  case MESSAGE_TYPE_A:
    Serial.printf("Message A received from %s\n", mac_str);
    ...
    Serial.printf("Data: %s and %d\n", msgA->a, msgA->b);

    break;
  case MESSAGE_TYPE_B:
    Serial.printf("Message B received from %s\n", mac_str);

    // Here it is a good idea to send the data for more processing to a queue and then process it in another task
    msgB = (MessageB *)incoming_data;
    Serial.printf("Data: %f and %d\n", msgB->c, msgB->d);

    break;
  default:
    Serial.printf("Message type not recognized from %s\n", mac_str);
    break;
  }
}
...
void setup()
{
  ...
  Serial.printf("ESP-NOW initialized. Waiting for messages... \n");
}
